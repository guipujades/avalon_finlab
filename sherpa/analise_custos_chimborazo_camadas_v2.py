...
def calcular_custos_primeira_camada(df_fundos, df_cadastro, valor_total_carteira, retorno_anual_estimado=0.12):
    ...
    for _, fundo in df_fundos.iterrows():
        ...
        # CORREÇÃO: Taxa de performance é cobrada sobre o RETORNO, não sobre o valor total
        # Assumindo um retorno anual estimado
        ...
        # Calcular custo total como % do valor investido
            ...
            try:
                ...
                if os.path.exists(cache_file):
                    ...
                    if (datetime.now().timestamp() - mod_time) < 86400:  # Cache de 24 horas
                        ...
                        # Cache expirado, processar novamente
                ...
                if not fundos_nivel2.empty:
                    ...
                    for _, fundo_n2 in fundos_nivel2.iterrows():
                        ...
                        # Calcular custos com o mesmo método corrigido
                        custo_adm_anual = valor_efetivo * taxas['taxa_adm']
                        
                        # Taxa de performance sobre o retorno estimado
...
def gerar_relatorio_custos(df_primeira_camada, df_segunda_camada, valor_total_carteira, base_dir):
    ...
    # Mostrar cálculo de performance para transparência
    ...
    print("\n\nRESUMO TOTAL:")
    print("="*60)
    custo_total_geral = custo_total_1a + custo_total_2a
    ...
    with pd.ExcelWriter(caminho_saida, engine='openpyxl') as writer:
        ...
        resumo = pd.DataFrame([
            ...
            {'Métrica': 'Custo Total 2ª Camada', 'Valor': custo_total_2a},
...
def plotar_analise_custos(df_primeira_camada, df_segunda_camada, base_dir):
    ...
    """
    # Configurar para salvar sem usar display
    ...
    if not df_segunda_camada.empty:
        dados_barras['2ª Camada'] = [df_segunda_camada['taxa_adm'].mean() * 100,
    ...
    ax4 = axes[1, 1]
    # Calcular taxa efetiva (custo total / valor investido)
    taxa_efetiva_1a = (df_primeira_camada['custo_total_anual'] / df_primeira_camada['valor_investido'] * 100)
    ax4.hist(taxa_efetiva_1a, bins=20, alpha=0.7, color='blue', label='1ª Camada')
    
    if not df_segunda_camada.empty:
        taxa_efetiva_2a = (df_segunda_camada['custo_total_anual'] / df_segunda_camada['valor_efetivo_chimborazo'] * 100)
        ax4.hist(taxa_efetiva_2a, bins=20, alpha=0.7, color='red', label='2ª Camada')
    
    ax4.set_xlabel('Taxa Efetiva (%)')
    ax4.set_ylabel('Frequência')
    ax4.set_title('Distribuição de Taxas Efetivas')
    ax4.legend()
