#!/usr/bin/env python3
"""
Corrige o layout da camada de manutenção para ocupar a janela inteira
e conserta o funcionamento do botão
"""
...
# Remover a estrutura atual da camada de manutenção
pattern = r'<!-- Camada de Manutenção -->.*?</div>\s*</div>\s*</div>\s*</div>\s*<!-- Camada de Ganho -->'

# Nova estrutura com layout correto
replacement = '''<!-- Camada de Manutenção -->
                ...
                <div class="mt-8">
                    <div class="bg-gradient-to-r from-orange-50 via-amber-50 to-yellow-50 rounded-xl p-8 border border-orange-200 shadow-lg">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <!-- Card PIMCO Income -->
                            <div class="etf-card bg-white rounded-xl p-6 shadow-md hover:shadow-xl transition-all duration-300 border border-gray-100 cursor-pointer" data-etf="pimco-income" onclick="window.open('https://www.pimco.com/en-us/investments/mutual-funds/income-fund', '_blank')">
...
# Agora vamos garantir que o event listener está no lugar correto
# Procurar o final dos event listeners e adicionar o de manutenção se não existir
if 'runMaintenanceAnalysis' not in content:
    # Adicionar após o fechamento do DOMContentLoaded
    pattern = r'(});)\s*</script>'
    
    new_listener = r'''});

        // Event listener para análise de manutenção
...
SGLN   -0.20 -0.10  1.00</pre>
    ...
    content = re.sub(pattern, new_listener, content, flags=re.DOTALL)

# Salvar o arquivo modificado
with open('gestao-ativa.html', 'w', encoding='utf-8') as f:
    f.write(content)

print("✓ Layout da camada de manutenção corrigido")
print("✓ Análise de produtos ocupa a janela inteira")
print("✓ Botão posicionado abaixo dos cards")
print("✓ Event listener do botão adicionado corretamente")