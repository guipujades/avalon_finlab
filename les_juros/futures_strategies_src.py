...
def handle_mt5_data(df):
    df.reset_index(inplace=True)
    df.columns = ['Data', 'Abertura', 'Maxima', 'Minima', 'Fechamento', 'tick_volume',
           'spread', 'Volume']
    df = df.iloc[0:,0:5]
    return df


def split_data(df_15m, df_30m, df_60m, test_size=0.7, random_state=42, date_max='2023-12-31'):

    dfs = {'15min': df_15m[df_15m.Data<=date_max], '30min': df_30m[df_30m.Data<=date_max], '60min': df_60m[df_60m.Data<=date_max]}
    
    for key in dfs:
        if 'Data' in dfs[key].columns:
            dfs[key] = dfs[key].sort_values('Data')
    
    # shuffle=False para manter a ordem cronologica
    resultado = {'test': {}, 'validation': {}}
    ...
    for key, df in dfs.items():
        ...
        resultado['test'][key] = X_train
        resultado['validation'][key] = X_validation
    
    return resultado


def identificar_primeira_barra_dia(df_60m):

    df_60m['Data_only'] = df_60m['Data'].dt.date
    primeira_barra = df_60m.groupby('Data_only').first().reset_index()
    
    primeira_barra['date_testRef'] = primeira_barra.Data + timedelta(hours=1)
    
    return primeira_barra


def calcular_movimento(df, idx, tamanho_barra_ref, timeframe):
    ...
    dados_periodo = df.iloc[idx:idx + n_barras + 1]
    
    if len(dados_periodo) < n_barras + 1:
        return None
    

    preco_inicio = dados_periodo.iloc[0]['Abertura']
    preco_fim = dados_periodo.iloc[-1]['Fechamento']
    movimento = preco_fim - preco_inicio
    
    movimento_normalizado = movimento / tamanho_barra_ref
    
    return {
        'direcao': 1 if movimento > 0 else -1,
        'amplitude_normalizada': abs(movimento_normalizado)
    }


def analisar_rompimentos(df_15m, df_30m, df_60m, primeira_barra):
    resultados = {
        'rompimento_maxima': {'15min': [], '30min': [], '60min': []},
        'rompimento_minima': {'15min': [], '30min': [], '60min': []}
    }
    
    for _, barra_60m in primeira_barra.iterrows():
        
        data_barra = barra_60m['date_testRef']
        fim_dia = datetime(data_barra.year, data_barra.month, data_barra.day, 23, 59, 59)
        maxima = barra_60m['Maxima']
        minima = barra_60m['Minima']
        tamanho_barra = maxima - minima
        
        for timeframe, df in [('15min', df_15m), ('30min', df_30m), ('60min', df_60m)]:    

            dados_posteriores = df[(df['Data'] >= data_barra) & (df['Data'] < fim_dia)]
            
            for idx, row in dados_posteriores.iterrows():
                
                if row['Fechamento'] > maxima:
                    movimento = calcular_movimento(df, idx, tamanho_barra, timeframe)
                    resultados['rompimento_maxima'][timeframe].append(movimento)
                    break
                    
                if row['Fechamento'] < minima:
                    movimento = calcular_movimento(df, idx, tamanho_barra, timeframe)
                    resultados['rompimento_minima'][timeframe].append(movimento)
                    break
...
def calcular_estatisticas(resultados):
    ...
    for tipo_rompimento in ['rompimento_maxima', 'rompimento_minima']:
        estatisticas[tipo_rompimento] = {}
        
        for timeframe in ['15min', '30min', '60min']:
            movimentos = [m for m in resultados[tipo_rompimento][timeframe] if m is not None]
            
            if movimentos:
                direcoes = [m['direcao'] for m in movimentos]
                amplitudes = [m['amplitude_normalizada'] for m in movimentos]
                
                prob_alta = sum(1 for d in direcoes if d > 0) / len(direcoes)
                
                estatisticas[tipo_rompimento][timeframe] = {
                    'probabilidade_alta': prob_alta,
                    'probabilidade_baixa': 1 - prob_alta,
                    'amplitude_media': np.mean(amplitudes),
                    'amplitude_mediana': np.median(amplitudes),
                    'total_ocorrencias': len(movimentos)
                }
...
MT5.initialize(user_path=str(mt5_path), server='GenialInvestimentos-PRD', login=156691, key='Avca@1985')

df_15m = handle_mt5_data(MT5.get_prices_mt5(symbol='WDO@', n=99999, timeframe=mt5.TIMEFRAME_M15))
df_30m = handle_mt5_data(MT5.get_prices_mt5(symbol='WDO@', n=99999, timeframe=mt5.TIMEFRAME_M30))
df_60m = handle_mt5_data(MT5.get_prices_mt5(symbol='WDO@', n=99999, timeframe=mt5.TIMEFRAME_H1))

dados_divididos = split_data(df_15m, df_30m, df_60m, date_max='2023-12-31')

df_15m_test = dados_divididos['test']['15min']
df_15m_validation = dados_divididos['validation']['15min']
df_30m_test = dados_divididos['test']['30min']
df_30m_validation = dados_divididos['validation']['30min']
df_60m_test = dados_divididos['test']['60min']
df_60m_validation = dados_divididos['validation']['60min']


primeira_barra = identificar_primeira_barra_dia(df_60m)

resultados = analisar_rompimentos(df_15m, df_30m, df_60m, primeira_barra)

estatisticas = calcular_estatisticas(resultados)



# df_nmin = MT5.get_prices_mt5(symbol='WDO@', n=99999, timeframe=mt5.TIMEFRAME_M1)
# df_nmin['Volume_Financeiro'] = ((df_nmin['Maxima'] + df_nmin['Minima']) / 2) * df_nmin['Volume']

# ops = Strategies.strategy_bbKeltner_marcius(df_h, df_nmin)

# trades = Backtester.run_backtest(df_nmin, ops, is_dol=True)
# metrics = Backtester.calculate_metrics(trades)
# df_trades = Backtester.create_trades_df(trades)

# plot_ = Visual.plot_results(df_trades)
