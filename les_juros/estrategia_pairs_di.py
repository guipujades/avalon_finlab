...
def run_di_pairs_strategy():
    """
    Função principal para executar a estratégia com dados do futures_v2.pkl
    """
    # Carrega dados
    ...
    try:
        ...
        print(f"Contratos disponíveis: {list(df.columns)[:10]}...")  # Mostra primeiros 10
        ...
        print(f"\nExecutando estratégia: {strategy.di_long} vs {strategy.di_short}")
        print(f"Janela: {strategy.window} dias")
        print(f"Desvio padrão: {strategy.n_std}")
        ...
        # Teste com outros pares
        print("\n=== TESTANDO OUTROS PARES ===")
        pares_teste = [
            ('DI1F24', 'DI1F25'),
            ('DI1F25', 'DI1F27'),
            ('DI1F26', 'DI1F28')
        ...
        for di_long, di_short in pares_teste:
            if di_long in df.columns and di_short in df.columns:
                strategy = DiPairsTrading(di_long, di_short, window=20, n_std=2.0)
                # Ajustar vencimentos conforme necessário
                results = strategy.backtest(df, venc_long, venc_short)
                if 'total_trades' in results['metrics']:
                    print(f"\n{di_long} vs {di_short}:")
        ...
        print("Certifique-se de que o arquivo futures_v2.pkl está na pasta les_juros")
    except Exception as e:
        print(f"Erro ao executar estratégia: {e}")


if __name__ == "__main__":
    run_di_pairs_strategy()